...
for k in range(loops):
    ...
    Movements = Parallel(n_jobs=LoopFiles)(delayed(SepreateMovements)(result[0], result[1])
    ...
    print(k, np.shape(Xmoves), (ti()-st)/60.0)
...
def splitLong(Moves,maxLength = 4000):
    ...
    for move in Moves:
        if len(move)> maxLength: 
            ...
            if len(move) < maxDesiredLength / 4:
                pass
            else:
                Xmoves.append(move)
    return Xmoves
...
lstm_autoencoder.model.save("LSTM_y")
