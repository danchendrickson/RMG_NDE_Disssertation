...
if arg_len > 3:
    singular = int(sys.argv[3])
else:
    singular = 0

if arg_len > 4:
    testMode = int(sys.argv[4])
else:
    testMode = 0
    
if arg_len > 5:
    startpoint = int(sys.argv[5])
else:
    startpoint = 0
...
if singular == 1:
    ...
    SaveFolder = '/scratch/1000Input/'
else:
    DataFolder = '/sciclone/scr10/dchendrickson01/Recordings2/'
    SaveFolder = '/sciclone/scr10/dchendrickson01/1000Input/'
...
def runFile(file, verbose = False, small = False, index=0, start=ti()):
    ...
    if file[-4:] == '.csv':    
        ...
        #df_training_value_h = df_h # df_training_value_h/training_std
        ...
        #df_training_value_v = df_v # df_training_value_v/training_std
        ...
        for i in range(dataLength):
            ...
            if verbose:
                if i % 100000 == 0:
                    print('Batch done', i, ti()-start)

            
        del x_train_ps#, x_train_hs, x_train_vs


        print(f'Done, {dataLength} files saved',index, file, ti()-start)


def runFileWrap(file, verbose, small, index):
    try:
        runFile(file, verbose, small, index)
    except:
...
for index in range(Groups-startpoint):
    start = StartPoint * Groups + startpoint
    if testMode == 1:
        Results = runFile(files[index+start], verbose, small, index)
    else:
        Results = runFileWrap(files[index+start], verbose, small, index)
