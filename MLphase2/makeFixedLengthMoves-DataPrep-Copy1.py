...
def create_sequences(Moves, time_steps=TIME_STEPS, skips = Skips, verbose = noise):
    count = 0
    output = []
    print(len(Moves))
    for move in Moves:
        if len(move) > time_steps:
            loops = int((len(move) - time_steps + skips)/skips)
            if loops > 1:
                loops -= 1
            for i in range(loops):
                output.append(move[i*skips : (i*skips + time_steps)])
                count+=1
        else:
            pass
    if count < 1:
...
def runFile(file, verbose = False, small = False, index=0, start=ti()):
    ...
    if file[-4:] == '.csv':    
        ...
        if verbose:
            print(f'Splint into {len(df_rs)}', ti()-start)        
        
        x_train_ps = create_sequences(df_ps, verbose)
        x_train_hs = create_sequences(df_hs)
        x_train_vs = create_sequences(df_vs)
        x_train_rs = create_sequences(df_rs)

        if verbose:
            print(f'{len(x_train_ps)} Sequences created', ti()-start)
        ...
        if len(x_train_ps) > 1:
            ...
            for i in range(dataLength):
                temp = np.matrix([x_train_p[i],x_train_h[i],x_train_v[i]]).T #
                np.savetxt(SaveFolder+file[:-4]+str(i).zfill(8)+'Data.csv', temp, delimiter=",")
                temp = np.matrix([x_train_ps[i]]).T
                np.savetxt(SaveFolder+file[:-4]+str(i).zfill(8)+'Outs.csv', temp, delimiter=",")
...
def getData(offset, loops = 20, starts=ti(), saveEvery=20., verbose=noise):
    Mats=[]
    counter = 0
    for i in range(loops):
        #try:
        FilesCreated = runFile(files[i+offset*loops], verbose = noise, small = False, index=0, start=starts)
        #except:
        #    print(f'failed on {files[i+offset*loops]}')
        ...
        for i in range(dataLength):
            ...
            '''                
        Mats = []
        print(f"{files[i+offset*loops]} loop {i} saved {FilesCreated} files",int((ti()-starts)/60*100)/100)
