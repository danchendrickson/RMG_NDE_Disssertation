...
Computer = "WinLap" # "SciClone"    "WinLap"  "LinLap"   "Desktop" 
...
for i in range(loops):
    ...
    if np.size(AllFingers) == 0:
        ...
        AllFingers = np.concatenate(AllFingers, Fingers, axis = 1)
...
model.add(ConvLSTM2D(filters = 32, 
            ...
            input_shape = (numberFrames, np.shape(X_train[0][0])[0], np.shape(X_train[0][0])[1],1)
            )
        )
