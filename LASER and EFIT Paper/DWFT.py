...
def FPFeatureVector(fp, obj=None, normHOG=True, HOGunsigned=True):
    ...
    try:
        ...
        featVec = np.append(featVec,'na')
    try:
        featVec = np.append(featVec, max(_RidgeCount1FP(tempFP)))
    except:
        featVec = np.append(featVec,'na')
...
def _RidgeCount1FP(fingerprint):
    '''
    Count the number of times the fingerprint changes from 0 to 1 or 1 to 0 in 
    consective rows. Gives a vector representation of the DWFT
    '''
    ...
    ridgeCount = np.max(diff.sum(axis=0))
...
def getProcessedFP(vec, wvt, ns=50, numslices=5, slice_thickness=0.12, threshold=50, graph=False):
