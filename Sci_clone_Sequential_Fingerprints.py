...
def makeFrames(input): #,sequ,frameLength):
    ...
    for i in range(numberFrames):
        ...
        w0=np.shape(imageMatrix)[0]
        a0=np.linspace(0,w0-1,w0,dtype=int)
        imageMatrix = np.delete(imageMatrix,a0,axis=0)

        w1=np.shape(imageMatrix)[0]
        a1=np.linspace(0,w1-1,w1,dtype=int)
...
# %%
accel = getAcceleration('60kPoints-220101-0708-s3.csv')
